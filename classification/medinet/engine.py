...
class MulticlassEngine(Engine):
    ...
    def on_end_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        # print('SHAPES', self.state['output'].data.cpu().numpy())
        # print(torch.sigmoid(-(self.state['output'][:, 0] - self.state['output'][:, 1])).data)
        # print('SHAPES', self.state['target'].cpu().numpy().shape)
        # self.state['auc'].add(torch.sigmoid(-(self.state['output'][:, 0] - self.state['output'][:, 1])).data, self.state['target'])
